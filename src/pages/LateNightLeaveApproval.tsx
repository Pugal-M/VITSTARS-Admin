...
export default function LateNightLeaveApproval() {
    ...
    const fetchRequests = async () => {
      try {
        ...
        const { data, error } = await supabase
          ...
          `)
          // Assuming there might be a specific type for late night, 
          // .eq('leave_type', 'Late Night')
  ...
  return (
    <div className="animate-fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--spacing-6)' }}>
        <div>
          <h1>Late Night Leave Approval</h1>
          <p style={{ color: 'var(--color-text-muted)', marginTop: '-1.25rem' }}>Approve, reject, and review student late night leave requests</p>
        </div>
      </div>
          ...
          <table className="admin-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} style={{ textAlign: 'center', padding: 'var(--spacing-8)' }}>No late night leave requests found.</td></tr>
              ) : (
                leaveRequests.map(req => (
                  <tr key={req.id}>
                    ...
                    <td>{req.leave_type || 'Late Night Leave'}</td>
                    <td>
